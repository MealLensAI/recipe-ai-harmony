...
DECLARE
  result JSONB;
BEGIN
  INSERT INTO public.profiles (id, email, firebase_uid, updated_at)
  VALUES (p_user_id, p_email, p_firebase_uid, NOW())
  ON CONFLICT (id) 
  DO UPDATE SET 
    email = EXCLUDED.email,
    firebase_uid = COALESCE(EXCLUDED.firebase_uid, profiles.firebase_uid),
    updated_at = NOW()
...
-- Add a detection history record
...
BEGIN
  INSERT INTO public.detection_history (
    user_id, detection_type, input_data, detected_foods, 
    recipe_suggestion, recipe_instructions, recipe_ingredients,
    analysis_id, youtube_link, google_link, resources_link
  )
  VALUES (
    p_user_id, p_detection_type, p_input_data, p_detected_foods,
    p_recipe_suggestion, p_recipe_instructions, p_recipe_ingredients,
    p_analysis_id, p_youtube_link, p_google_link, p_resources_link
  )

...
const AIResponsePage: FC = () => {
  ...
  const handleDiscoverRecipes = async () => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:7017/process", {
        method: "POST",
        body: formData,
      })
  ...
  const handleHealthMealGeneration = async () => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:7017/generate_meals_from_ingredients", {
        method: "POST",
        body: formData,
      })
  ...
  const handleSuggestionClick = async (suggestion: string) => {
    ...
    try {
      ...
      const instrRes = await fetch("http://127.0.0.1:7017/instructions", {
        method: "POST",
        body: formData,
      })
  ...
  const handleHealthMealInstructions = async (meal: HealthMeal) => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:7017/sick_meal_plan_instructions", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
